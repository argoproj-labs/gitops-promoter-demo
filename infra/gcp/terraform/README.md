...
2. Edit **`./infra/gcp/terraform/terraform.tfvars`** (`project_id`, `project_name`, `billing_account`). For a fresh tree, you can start from `terraform.tfvars.example`. Region, VPC layout, and node sizing are **`locals`** at the top of `main.tf`. To create the project under a GCP org/folder, add `org_id` / `folder_id` to the `google_project` resource there.
